...
@section('contentheader_description')
  Scan Kanban Untuk Melakukan Delivery
@endsection
            ...
            <div class="card">
              <div class="form-row">
                <div class="form-group col-md-4">
                  <label for="scan">Scan Customer</label>
                  <input type="text" id="customer" name="scan" class="form-control" placeholder="No Kanban">
                </div>
                <div class="form-group col-md-4">
                  <label for="scan">Scan Cycle</label>
                  <input type="text" id="cycle" name="scan" class="form-control" placeholder="No Kanban">
                </div>
                <div class="form-group col-md-4">
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    function sendAjax(customer, cycle, scan) {
      $.ajax({
        type: 'post',
        url: "{{ route('manual-delivery') }}",
        dataType: 'json',
          data: {
            _token: "{{ csrf_token() }}",
            customer : customer,
      ...
      $('#cycle').on('keypress', function(event) {
        if (event.keyCode == 13) {
          $('#scan').focus();
        }
      });
      ...
      $('#scan').on('keypress', function(event) {
        if (event.keyCode == 13) {
          sendAjax($('#customer').val(), $('#cycle').val(), $('#scan').val());
